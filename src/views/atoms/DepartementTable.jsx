...
import { tableHeadDepartement } from '../../utils/tableHead';
...
const DepartementTable = () => {
  ...
  const getDepartement = async () => {
    const response = await axios.get('http://localhost:3000/departement');
    const { data } = response;
    setDatas(data.payload.data);
  };

  const handleDelete = async (id) => {
    const res = await axios.delete(`http://localhost:3000/departement/${id}`);
    console.log(res);
  ...
  return (
    <div className="mt-10 px-4">
      <Button
        ...
        onClick={() => navigate('/add_departement')}
      >
        Add Departement
      </Button>
